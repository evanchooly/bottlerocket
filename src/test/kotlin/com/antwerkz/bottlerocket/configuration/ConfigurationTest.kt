    ...
    fun testYaml() {
        val configuration = configuration {
            systemLog {
                ...
                component {
                    systemLog {}
                    accessControl { verbosity = Verbosity.FIVE }
                }
            }
        }
        val target =
        ...
        val configuration = configuration {
            ...
            systemLog {
                ...
                component {
                    net {}
                    storage {}
                    accessControl { verbosity = Verbosity.TWO }
                }
            }
...
"""
    ...
    fun testBuilder() {
        val config = configuration {
            systemLog {
                destination = Destination.SYSLOG
                component { accessControl { verbosity = Verbosity.FIVE } }
            }
        }
        Assert.assertEquals(
            config.toYaml(),
...
"""
    ...
    fun testComplexBuilder() {
        val configuration = configuration {
            ...
            systemLog {
                ...
                component { accessControl { verbosity = Verbosity.TWO } }
            }
            processManagement { fork = true }
        }
    ...
    @Test
    fun smallMerges() {
        var config = configuration { auditLog { filter = "filter value" } }
        var update: Configuration = config.update { auditLog { filter = "updated" } }

        Assert.assertEquals(config.auditLog?.filter, "filter value")
        Assert.assertEquals(update.auditLog?.filter, "updated")

        config = configuration { net { port = 12345 } }

        update = config.update { net { port = 49152 } }
    ...
    fun mergeConfig() {
        val config = configuration {
            auditLog { filter = "filter value" }
            storage { dbPath = "/var/lib/mongo/noodle" }
            net { port = 12345 }
        }
