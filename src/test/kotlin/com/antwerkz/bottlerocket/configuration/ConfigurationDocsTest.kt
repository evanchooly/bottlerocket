...
class ConfigurationDocsTest : BaseTest() {
    ...
    fun checkDocs(version: Version) {
        ...
        Assert.assertTrue(
            elements.isEmpty(),
            "found ${elements.size} extra items in ${version.docsUrl()}: \n${
        ...
        Assert.assertTrue(
            missing.isEmpty(),
            "found ${missing.size} extra items in ${version.docsUrl()}: \n${missing.joinToString("\n")}"
        )
    }
    ...
    fun propertyMap(type: Class<*>): Map<String, Field> {
        ...
        type.declaredFields.forEach { field ->
            if (ConfigBlock::class.java.isAssignableFrom(field.type)) {
                ...
                fieldMap.forEach { (key, value) -> map["${field.name}.$key"] = value }
            } else {
    ...
    private fun check(
        ...
        map.forEach { (key, value) ->
            ...
            var annotated = false
            if (!present) {
                annotated = wasRemoved(value, version) || wasAdded(value, version)
            }
            if (!present && !annotated)
                outdated += "Found $key in the model but not in the configuration: $url"
        }

        Assert.assertTrue(
            outdated.isEmpty(),
            "Found mismatches between docs and code: " + outdated.joinToString("\n", "\n")
        )
        return outdated
    }
    ...
    private fun Version.docsUrl() =
        "http://docs.mongodb.org/v$majorVersion.$minorVersion/reference/configuration-options/"
}
